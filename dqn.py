...
class POMDP_solver(RL_Algorithm):
    def __init__(self, mdp: MDP, flag_folder):
        self.mdp = mdp
        self.mdp.R = np.sum(self.mdp.R * self.mdp.P, axis=2).T # (a,s,s_new)
        self.n_states = mdp.n_states
        self.flagdir = "./{}/{}.txt".format(flag_folder, str(uuid.uuid4()))
...
class MDP_env(gym.Env):
    ...
    def __init__(self, n_states, beta, mdp, horizon):
        super().__init__()
        # Other parameters
        self.n_states = n_states
        self.t_cost = beta
        ...
        self.R = mdp.R
        self.gamma = mdp.gamma
        # List of actions and action space
        self.actions = self.init_A()
        self.nA = len(self.actions)
        self.action_space = gym.spaces.Discrete(self.nA)
        # Observation space
        self.observation_space = gym.spaces.Box(low=0, high=1, shape=(n_states - 1,), dtype=np.float32)
        self.b0 = np.zeros((self.n_states,1))
        self.b0[0,0] = 1.0
        self.b = self.b0
        # Utils
        # self.pr_o_sa = self._init_Pr_o_sa()
        self.episode_length = horizon
        self.t = 0
    
    def init_A(self): # CHECKED
        decisions = ([set([0,1])] * np.shape(self.P)[0])
        return np.array(list(product(*decisions)))            
    
    # def _init_Pr_o_sa(self): 
    #     nO = self.n_states + 1
    #     probs = np.zeros((nO, self.n_states, self.nA))
    #     for s, ind_a in product(range(self.n_states), range(self.nA)):
    #         a_tx, a_rx = self.decode_action(ind_a)

    #         # probabilities next state
    #         probs_next_state = self.P[a_rx, s] 

    #         # probabilities of observing a state
    #         for ind_o, state_prob, state_active in zip(range(self.n_states), probs_next_state, a_tx):
    #             probs[ind_o, s, ind_a] = state_active * state_prob

    #         # probability of observing nothing
    #         probs[self.n_states, s, ind_a] = np.dot(1-np.array(a_tx), probs_next_state)
    #     return probs
    
    def decode_action(self, a):
        """The POMDP actions are in the form (0/1, 0/1, ..., 0/1, 0/1/2/3/../n_actions-1), where the last element is the
        action taken by the receiver and the first ones are the decision rule of the transmitter."""
        n_actions = np.shape(self.P)[0]
        a_tx = a // n_actions
        string_representation_of_a_tx = f'{a_tx:0{self.n_states}b}' 
        a_tx = [int(bit) for bit in string_representation_of_a_tx]
        a_rx = a % n_actions
    ...
    def R_b(self, b, a):
        ...
        def R_s(s):
            """one step reward, including the expected transmission costs"""
            immediate_reward = self.R[s, a_rx] # immmediate reward from taking action a_rx in state a
            transmissions = np.dot(self.P[a_rx,s], np.array(a_tx)) # probability of transmission
            return immediate_reward - self.gamma * transmissions * self.t_cost
        return np.dot(np.array([R_s(s) for s in range(self.n_states)]), np.squeeze(b))

    def update_b(self, b, a, o):
        assert type(o) == int
        a_tx, a_rx = self.decode_action(a)
        # if a message was received
        if o < self.n_states:
            if a_tx[o] == 0:
                raise Exception("Action corresponding to observed y is 'silence'.")
            tmp = np.zeros((self.n_states,1))
            tmp[o,0] = 1.0
            return tmp

        # if no message was received
        if np.sum(a_tx) == self.n_states:
            raise Exception("There are no silent states.")
        stepped_b = self.P[a_rx].T @ b
        tmp = np.expand_dims((1 - np.array(a_tx)), axis=1)
        b_unnormalised = tmp * stepped_b
        return b_unnormalised / np.sum(b_unnormalised)

    def step(self, a):
        a = int(a)
        a_tx, a_rx = self.decode_action(a)
        state_sample = int(random.choices(np.arange(self.n_states), self.b)[0])
        next_state_probs = self.P[a_rx, state_sample]
        sampled_state = int(random.choices(np.arange(self.n_states), next_state_probs)[0])
        if a_tx[sampled_state] == 1:
            sampled_observation = sampled_state
        else:
            sampled_observation = self.n_states
        # if sampled_observation < self.n_states:
        #     if a_tx[sampled_observation] == 0:
        #         raise Exception("This shouldn't be possible.")
        assert np.sum(self.b) > 0.95 and np.sum(self.b) < 1.05
        new_b = copy.deepcopy(self.update_b(self.b, a, sampled_observation))
        r = self.R_b(self.b, a)
        self.t += 1
        done = self.t == self.episode_length
        self.b = new_b
        if not (np.sum(self.b) > 0.95 and np.sum(self.b) < 1.05):
            pass
        return np.squeeze(new_b.astype(np.float32)[:self.n_states - 1]), r, False, done, {}

    def reset(self, seed=None, options=None):
        self.b = copy.deepcopy(self.b0)
        assert np.sum(self.b) > 0.95 and np.sum(self.b) < 1.05
        self.t = 0
        return np.squeeze(self.b.astype(np.float32)[:self.n_states - 1]), {}
...
class Deep_RL_Tx:
    def __init__(self, model, n_states, solver_env):
        self.model = model
        self.n_s = n_states
...
def DQN_agent_update_b(b, a_ind, o, n_states, solver_env):
    if o is None:
        o_ind = n_states
    else:
        o_ind = int(o)
    return copy.deepcopy(solver_env.update_b(b, a_ind, o_ind))

def DQN_get_action(model, b, A):
    a_ind = int(model.predict(np.squeeze(b)[:-1], deterministic=True)[0])
    a = A[a_ind]
    return a, a_ind


class DQN_solver(RL_Algorithm):
    def __init__(self, mdp: MDP, save_path):
        self.mdp = mdp
        self.mdp.R = np.sum(self.mdp.R * self.mdp.P, axis=2).T # (a,s,s_new)
        self.n_states = mdp.n_states
    ...
    def run(self, beta, horizon):
        self.beta = beta # MAYBE NOT THE BEST WAY

        # create the POMDP interface and declare the solver
        if not np.all(np.isclose(np.sum(self.mdp.P, axis=2), 1.0)):
            raise Exception("2nd axis of P does not sum to 1")
        self.DQN_model = MDP_env(self.n_states, beta, self.mdp, horizon)
        env_checker.check_env(self.DQN_model, warn=True, skip_render_check=True)

        monitor = Monitor(self.DQN_model, None)

        # Callbacks
        # 30 5 25000
        stop_train_callback = StopTrainingOnNoModelImprovement(max_no_improvement_evals=30, min_evals=5, verbose=0)
        eval_callback = EvalCallback( monitor, 
                                      eval_freq=25000, 
    ...
    for experiment_type in experiment_types:
        ...
        for mdp_i, mdp in enumerate(mdps):
            ...
            for beta in betas:
                ...
                if mdp_i == 0:
                    checkpoint_folder = create_folder(results_folder + "/" + str(beta))
                else:
                    checkpoint_folder = results_folder + "/" + str(beta)
